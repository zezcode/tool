var soft = {
    ...
    set_soft_tips:function(rdata,type){
        ...
        if(type == 10){
            ...
            explain.text('Dưới đây là ứng dụng của Bên Thứ 3. Không thể bảo đảm tính an toàn, Khuyến cáo không nên sử dụng ');
            btn_ground = soft.render_tips_btn(btn_ground,[
                {title:'Zalo Admin',rel:'noreferrer noopener',href:'https://zalo.me/0965781640',target:'_blank',btn:'Zalo Admin',class:'btn btn-success btn-xs va0 ml15',style:"margin-left:10px;"},
                {title:'Import Plugins',href:'javascript:;',btn:'Nhập Plugin riêng của bạn','class':'btn btn-success btn-xs va0 ml15','style':"margin-left:10px;",click:function(e){
            ...
            if((ltd > 0 && (ltd == pro || pro < 0)) || (ltd < 0 && pro >= 0) || (ltd > 0 && pro >= 0)){
                ...
                explain.html('Phiên bản '+ (_ltd?'Pro':'Premium') +' mở khóa '+ (_ltd?'专业版及企业版插件':'Tất Cả Tính Năng. ') + (!(pro == 0 && ltd < 0)?('expiration time: '+ (bt.format_data((_ltd?ltd:pro),'yyyy/MM/dd') ) +''+((((_ltd?ltd:pro) - todayDate) <= 15*24*60*60)?('，<span style="color:red">Only '+ Math.round(((_ltd?ltd:pro) - todayDate) / (24*60*60)) +' days until expiration</span>'):'')):'Thời hạn sử dụng: <span style="color: #fc6d26;font-weight: bold;">Vĩnh Viễn</span>'));
            }else if(ltd == -1 && pro == -1){
                explain.html('Upgrade to Pro edition, all plugins, free to use!');
            }else if(pro == 0 && ltd < 0){
                _ltd = 2;
                explain.html('Bản Crack Premium aaPanel '+(type == 12?'&nbsp;&nbsp;<span style="color:#af8e48">đã mở khóa tất cả tính năng và ứng dụng mất phí.</span>':''));
                if(type == 12){
            ...
            var set_btn_style = function(res){
              if(!res.status || !res){
                ...
                if(type == 12 && (ltd < 0 && pro >=0)){
                  explain.html('Phiên bản crack Premium có thể sử dụng tất cả tính năng mất phí. Để hiểu rõ hơn hãy nhấp <a href="https://www.bt.cn/download/linux.html" target="_blank" class="btlink ml5">Vào Đây</a> hoặc<a href="https://zalo.me/hieudz3101" target="_blank" class="btlink ml5">《Liên hệ Admin》</a>');
              ...
              if(_this.trail){
                setTimeout(function (){
                  $('.btn-ground').after('<span class="pro_trail" style="font-weight: 700;margin-left:25px;">Nếu cần hỗ trợ </span>')
                  var trail = $('<a href=\"https://zalo.me/0965781640" style="color: #fc6d26;font-weight: bold;">Hãy ấn vào đây</a>');
                  trail.click((!res.status || !res)?fun:function(){
                    var loadT = bt.load()
                    bt.confirm({
                      title:"Pro Edition",
                      msg:"Đã mở khóa Pro Edition"
                    },function (){
